...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_existing_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 45,
            'price': Decimal('8.25'),
            'ingredients': [{'name': 'Salami'}, {'name': 'Mozzarella'}]
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.ingredients.count(), 2)
        self.assertIn(ingredient_pizza, recipe.ingredients.all())
        for ingredient in payload['ingredients']:
            exists = recipe.ingredients.filter(
                name=ingredient['name'],
                user=self.user,
    ...
    def test_create_ingredient_on_update(self):
        ...
        payload = {'ingredients': [{'name': 'Pistachios'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        ingredient_ham = Ingredient.objects.create(user=self.user, name='Ham')
        recipe = create_recipe(user=self.user)
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(ingredient_oregano, recipe.ingredients.all())
        self.assertNotIn(ingredient_ham, recipe.ingredients.all())
